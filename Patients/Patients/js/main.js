...
const handleSearchSurnameStatus = (event) => {
    ...
    filteredResults.forEach(patient => {
        if (selectActiveCheckBox.checked === JSON.parse(patient.patient_status)) {
    ...
    if (searchSurname.value.length > 0) {
        filteredResults = filteredResults.filter(patient => patient.patient_surname.toLowerCase().includes(searchSurname.value.toLowerCase()));          
    }
